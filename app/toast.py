...
class ToastHost:
    ...
    def __init__(self, page: ft.Page):
        ...
        self.icon = ft.Icon(ft.Icons.CHECK, size=18, color=C.OK)
        self.text = T("", size=13, color=C.TEXT_1, expand=True,
                      max_lines=2, overflow=ft.TextOverflow.ELLIPSIS)
        self.action_label = T("", size=12.5, weight=ft.FontWeight.W_600, color=C.TEXT_1)
        self.action_btn = ft.Container(
            self.action_label, visible=False, padding=ft.padding.only(0, 0, 0, 2),
            border=ft.border.only(bottom=ft.BorderSide(1, C.TEXT_5)),
            on_click=lambda e: self.fire_action())
        self.countdown = T("", size=10.5, color=C.TEXT_4, font_family="monospace", visible=False)
        ...
        self.card = ft.Container(
            ft.Row([self.icon, self.text, self.action_btn, self.countdown],
                   spacing=12, vertical_alignment=ft.CrossAxisAlignment.CENTER,
                   tight=True),
            bgcolor=C.BG_HOVER, border=ft.border.all(1, C.BORDER_ACTIVE),
            border_radius=12, padding=ft.padding.symmetric(13, 16), shadow=ft.BoxShadow(blur_radius=50, spread_radius=0,
                                            offset=ft.Offset(0, 20), color=C.SHADOW_BAR),
    ...
    # ---- public ----
    def show(self, text: str, icon=ft.Icons.CHECK, icon_color=C.OK,
        ...
        with self._lock:
            ...
            self.icon.color = C.ERR if error else icon_color
            self.text.value = text
            self.card.bgcolor = C.ERR_BG if error else C.BG_HOVER
            self.card.border = ft.border.all(1, C.ERR_BORDER if error else C.BORDER_ACTIVE)
            self.action_btn.visible = bool(action)
            self.action_label.value = action_label or "Отменить"
            ...
            self.card.width = (C.TOAST_MIN_W if len(text) <= 48 else C.TOAST_MAX_W)
            self.control.visible = True
            self._arm(1.0 if action else PLAIN_SECONDS, token)
        self._safe_update()

    def error(self, text: str, action=None, action_label: str | None = None):
        self.show(text, error=True, action=action, action_label=action_label)
